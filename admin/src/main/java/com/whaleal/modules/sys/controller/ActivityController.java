...
public class ActivityController {
    ...
    public Result<List<ActivityEntity>> list(@RequestParam Long id) {
        List<ActivityEntity> list = activityService.listAllById(id);

        return new Result<List<ActivityEntity>>().ok(list);
    }

    @PostMapping("/create")
    @Operation(summary = "创建一条跟进记录")
    @RequiresPermissions("activity:create")
    public Result createActivity(@RequestBody ActivityEntity activityEntity){
